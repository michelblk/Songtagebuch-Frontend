...
export class AddDiaryEntryComponent implements OnInit, OnDestroy {
  ...
  constructor(private songService: SongService, private diaryEntryService: DiaryEntryService) {
  }
  ...
  loadSuggestions(event: any, song: Partial<Song>) {
    if(this.searchRequestOngoing$.getValue()){return} // TODO better rate limiting

    this.searchRequestOngoing$.next(true);
    this.songService.search(event.target.value)
      .pipe(
  ...
  submit() {
    ...
    this.diaryEntryService.add(`78b5b2a8-a7e5-477e-b882-9095ebeedd16`, entry) //FIXME
      .pipe(
        takeUntil(this.destroyNotifier$)
      )
      .subscribe(success => {
